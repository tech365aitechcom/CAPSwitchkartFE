...
const Login = () => {
        ...
        axios
          ...
          .then(async () => {
            ...
            try {
              ...
              moduleConfig = moduleRes.data
              sessionStorage.setItem(
                'moduleConfig',
                JSON.stringify(moduleConfig),
              )
            } catch (_) {}
            redirectUser(storedProfile.role, moduleConfig)
          })
          .catch(() => {
  ...
  const getSuperAdminHome = (moduleConfig) => {
    if (!moduleConfig) return '/adminmodels'
    const order = [
      { key: 'adminModels', route: '/adminmodels' },
      { key: 'gradePricing', route: '/gradepricingsheet' },
      { key: 'registerUser', route: '/registeruser' },
      { key: 'createCoupon', route: '/createcoupon' },
      { key: 'bulkUploadHistory', route: '/bulkuploadhistory' },
      { key: 'storeListing', route: '/storelisting' },
      { key: 'storeReport', route: '/storewisereport' },
      { key: 'quoteTracking', route: '/quotetrackingdashboard' },
      { key: 'customerTable', route: '/customertable' },
      { key: 'companyListing', route: '/companylisting' },
      { key: 'pickupCancelDevice', route: '/devicepickupdashboard' },
      { key: 'technicianReport', route: '/technicianwisereport' },
      { key: 'adminDashboard', route: '/admindashboard' },
    ]
    const first = order.find((item) => moduleConfig[item.key] === true)
    return first ? first.route : '/profile'
  }

  const redirectUser = (role, moduleConfig) => {
    const customTable = '/customertable'
    switch (role) {
      case 'Super Admin':
        navigate(getSuperAdminHome(moduleConfig))
        break
      case 'Admin Manager':
      case 'Super_Admin_Unicorn':
      case 'Admin_Manager_Unicorn':
    ...
    axios
      ...
      .then(async (response) => {
        ...
        try {
          ...
          moduleConfig = moduleRes.data
          sessionStorage.setItem('moduleConfig', JSON.stringify(moduleConfig))
        } catch (_) {
          // non-fatal
        }

        redirectUser(profile.role, moduleConfig)
      })
      .catch((err) => {
